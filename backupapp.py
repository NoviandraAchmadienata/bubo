...
# Keep CPU inference logs readable on machines where NNPACK is unavailable.
...
class DetectionService:
    ...
    def _open_capture(self, cv2):
        ...
        if not capture.isOpened():
            capture.release()
            raise RuntimeError("Tidak dapat membuka stream RTSP. Periksa jaringan/URL.")
        return capture
    ...
    def _run(self):
        ...
        try:
            ...
            if ENABLE_YOLO:
                self._load_model()
                use_half = self.stats.device == "cuda"
            else:
            ...
            while self.running:
                ...
                if not ret or frame is None:
                    self._set_error("Frame kosong dari RTSP. Mencoba koneksi ulang.")
                    capture.release()
                    capture = None
                    time.sleep(1.0)
                    continue
...
@app.route("/")
def dashboard():
    return render_template("dashboard.html", config=detector.snapshot())


@app.route("/cctv")
def cctv_page():
    return render_template("cctv.html", config=detector.snapshot())
...
def video_feed():
    ...
    def generate():
        last_frame = None
        while True:
            frame = detector.wait_for_frame(last_frame)
            if frame is None:
                time.sleep(0.2)
                continue
